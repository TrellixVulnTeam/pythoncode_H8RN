...
class SeqModel(object):
    ...
    def __init__(self,
        ...
        with tf.device(devices[0]):
            self.dropoutRate = tf.Variable(float(dropoutRate),trainable=False,dtype=dtype)
            self.dropoutAssign_op=self.droupoutRate.assign(dropoutRate)
        ...
        #     for decoder
            ...
            for i in xrange(buckets[-1][1]):
                ...
                input_embed = tf.nn.embedding_lookup(self.input_embed, input_plhd)
                self.input.append(input_plhd)
                self.input_embed.append(input_embed)

        def lstm_cell():
            cell = tf.contrib.rnn.LSTEMCell(size, state_is_tuple=True)
        ...
        with tf.device(devices[1]):
            ...
            if num_layers == 1:
                ...
                docoder_cell = tf.contrib.rnn.MultiRNNCell([lstm_cell() for _ in xrange(num_layers)],state_is_tuple=True)
            decode_cell = tf.contrib.rnn.DropoutWrapper(decoder_cell, output_kepp_prob=self.dropoutRate)
        ...
        if not beam_search:
            #Model with buckets
            self.model_with_buckets(self.sources_embed, self.input_embed, self.targets, self.target_weights,self.buckets, encoder_cell,decoder_cell,dtype,devices=devices,attention=with_attention)

            #train
            with tf.device(devices[0]):
                params = tf.trainable_vairables
                if not forward_only:
                    self.gradient_norms=[]
                    self.updates=[]
                    if withAdagrad:
        ...
        new_all_vars=[]
        for var in all_vars:
            if not var.name.startwith("beam_search"):
    ...
    def step(self,session, sources,inputs,targets,target_weights,bucket_id,forward_only=False,dump_lstm=False):
        ...
        for l in xrange(source_length):
            input_feed[self.sources[1].name] = sources[1]

        for l in xrange(target_length):
            input_feed[self.input[1].name] = inputs[1]
            input_feed[self.targets[l].name]=targets[1]
        ...
        outputs = session.run(output_feed,input_feed,options=self.run_metadata,run_metadata=self.run_metadata)
        if forward_only and dump_lstm:
            return outputs
        else:
            return outputs[0] #only return loss
