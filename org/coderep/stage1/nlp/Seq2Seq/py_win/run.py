...
def train():
    ...
    ## train_n_tokens=total_training_target_size
    ##因为是二维数组，所以 两轮np.sum
    ## np.sum可以求list的和
    ...
    total_steps=steps_per_epoch * n_epoch
    '''
     记录共train_total_size条，每次(步)批量执行batch_size条，那么共有steps_per_epoch步,没走到一半，执行checkpoint
     因为有n个epoch，那么共有total_steps= steps_per_epoch * n_epoch
    '''
    ...
    mylog("bocket sizes:{}".format(dev_bucket_sizes))
    ...
    mylog_section("IN TENSORFLOW")
    config =tf.ConfigProto(allow_soft_plachment=True,log_device_placement=False)
    config.gpu_options.allow_growth=FLAGS.allow_growth
    ...
    with tf.Session(config=config) as sess:
        ...
        # statistics during training
            ...
            while current_step < total_steps
                ...
                #time
                step_time += (time.time() - start_time)/steps_per_epoch
                ...
                if current_step % steps_per_report==0:
                    ...
                    if FLAGS.profile:
                        ...
                        ctf = t1.generate_chrome_trance_format()
                 ...
                 if current_step % steps_per_checkpoint == 0:
                    ...
                    #report
                    sect_time = "CHECKPOINT {} STEP {}".format(i_checkpoint, current_step)
                ...
                if patience <=0:
                    mylog("Training finished. Running out if patience.")
                    break
